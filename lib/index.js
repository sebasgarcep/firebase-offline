  ...
  const getActions = () => {
    ...
    Object.keys(app.actions).forEach((key) => {
      ...
      if (config.validate) {
        ...
        const validationCallback = params =>
      ...
      actions[key] = params => async (dispatch, getState) => {
        await middleware.reduce((acc, mid) =>
          acc.then(() => mid(params)), Promise.resolve(true));

        return config.handler(params)(dispatch, getState);
      };
    });
